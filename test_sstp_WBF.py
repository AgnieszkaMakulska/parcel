...
def make_figure(sstp):
    ...
    for mixing in [True, False]:
        ...
        RH, T, rv, z, ice_mix_ratio, liq_mix_ratio, ice_conc, act_conc, ice_r, act_r = run_scheme(outfile, mixing, sstp)            

        (ice_c, liq_c, ice_l, liq_l) = ("skyblue", "coral", "ice (coupled)", "liquid (coupled)") if mixing else ("steelblue", "sienna", "ice (uncoupled)", "liquid (uncoupled)")
        (c, l) = ("violet", "coupled") if mixing else ("purple", "uncoupled")

        ax[0,0].plot(ice_mix_ratio * 1e3, z, color=ice_c, label=ice_l)
        ax[0,0].plot(liq_mix_ratio * 1e3, z, color=liq_c, label=liq_l)
